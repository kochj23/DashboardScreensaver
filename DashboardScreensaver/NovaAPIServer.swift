...
class NovaAPIServer {
    ...
    func start() {
        do {
            ...
            listener?.stateUpdateHandler = { if case .ready = $0 { print("NovaAPI [DashboardScreensaver]: port \(self.port)") } }
            listener?.start(queue: .main)
        } catch { print("NovaAPI [DashboardScreensaver]: failed — \(error)") }
    }
    ...
    private func route(_ req: NovaRequest) async -> String {
        ...
        case ("GET", "/api/status"):
            return json(200, ["status": "running", "app": "DashboardScreensaver", "version": "1.0", "port": "\(port)", "uptimeSeconds": Int(Date().timeIntervalSince(startTime))] as [String: Any])
        case ("GET", "/api/ping"):
